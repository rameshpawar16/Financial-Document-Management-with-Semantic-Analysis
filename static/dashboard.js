...
async function loadDashboard(){
  ...
  try {
    ...
    document.getElementById("currentUser").innerText = user.name || user["name"] || user.email
    const nameStr = user.name || user["name"] || user.email || "U"
    document.getElementById("avatarLetter").innerText = nameStr.charAt(0).toUpperCase()
...
async function uploadDoc(){
  ...
  try {
    ...
    if(response.ok){
      ...
      statusDiv.innerText = "Upload failed: " + (data.detail || "Unknown error")
      statusDiv.style.color = "red"
    }
  } catch(err) {
...
async function showDocuments(){
  ...
  try {
    ...
    let html = '<table class="doc-table"><tr><th>ID</th><th>Title</th><th>Company</th><th>Type</th><th>Uploaded By</th><th>Action</th></tr>'
    docs.forEach(doc => {
      html += `<tr>
    ...
    docListDiv.innerHTML = "<p style='color:red'>Failed to load documents: " + err.message + "</p>"
  }
}
...
async function askQuestion(){
  ...
  try {
    ...
    const data = await response.json()
    const lb = document.getElementById("loadingBubble"); if(lb) lb.remove()
    ...
    if(response.ok){
      if(data.results && data.results.length > 0){
        const resultsHtml = data.results.map((r, i) => {
          ...
          const score = r.score ? `<span class="score">relevance: ${r.score.toFixed(2)}</span>` : ""
